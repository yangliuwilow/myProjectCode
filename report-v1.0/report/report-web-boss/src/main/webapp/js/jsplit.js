	...
	jsplit:function(j){
		...
		$(_self).wrapInner("<div class='jsplit-c' style='top:0px;z-index:9999;zoom:1;width:100%;overflow:hidden;position:relative;height:100%'></div>");
			...
			jsplite.mousedown(function(e){
				...
				var screenX = e.screenX, w = $(_self).width(),mk=$("#mainDiv").width();
				...
				if(Close==true){
					...
					//$(_self).animate({width:dw},100);
					$(_self).css({width:dw}).show(1000);
					var by=document.body.clientWidth;
					$("#mainDiv").css({width:by-dw});
					Close=false;
				};
				return false;
			});
			...
			function _selfclose(){
					...
					jsplith.css({background:j.Btn.cBg.Out,"background-image":j.BgUrl});
					var mk=$("#mainDiv").width()
					$("#mainDiv").css({width:mk+$(_self).width()});
